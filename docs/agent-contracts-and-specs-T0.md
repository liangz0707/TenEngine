...
- **工作流**：拉取 T0-contracts → 读依赖契约 → 实现 → 改接口时**在 ABI 文件中增补或替换对应的 ABI 条目**再同步契约，并通知下游。plan 生成全量 ABI 内容用于实现，但文档中只保存新增/修改部分，写回时也仅写入该部分。
- **契约更新流程（ABI 先行）**：模块对外接口须在 **ABI 文件**（`NNN-modulename-ABI.md`）中**增补或替换**对应的 ABI 条目（每行须完整）；若下游需要某接口而上游尚未提供，须在**上游模块的 ABI 文件**中增加该接口的 **TODO** 条目，待实现时转为正式行。见 `specs/_contracts/README.md`「契约更新流程」。
