...
- **工作流**：拉取 T0-contracts → 读依赖契约 → 实现 → 改接口时更新契约并通知下游。
- **契约写法**：文件命名、建议结构（适用模块、消费者、版本/ABI、类型与句柄、能力列表、调用顺序与约束、变更记录）；接口符号以对应 ABI 文件为准。
- **接口演进**：契约（能力与类型）+ ABI 文件（命名空间、头文件、符号）→ 真实 API（头文件）。
- **发现依赖变化**：工作前、每个 task 开始前拉取 T0-contracts；收到 follow-up 时先拉再适配。
