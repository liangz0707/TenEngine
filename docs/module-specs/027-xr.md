...
Session：IXRSession::BeginSession、EndSession；CreateXRSession、XRSessionDesc。  
Frame：IXRSession::BeginFrame、EndFrame、GetViewCount、GetViewport、GetProjection；Viewport。  
Submit：IXRSession::Submit、GetSwapChain；与 RHI、pipeline-to-rci 约定一致。  
Input（可选）：IXRSession::GetControllerPose、GetHeadPose；与 Input 模块扩展对接。
