...
- **组件**：组件挂载、组件查询、依赖 Entity 与 Scene 的变换与层级；**ModelComponent** 持有 **ResourceId（modelResourceId）**，005 不持有 IModelResource*、对 IResource 不可见；Entity创建时自动从Scene节点读取ResourceId并创建ModelComponent；渲染时由 Pipeline 经 013 解析后使用。
...
| Component | 组件注册、挂载/卸载、组件类型查询、与 Object 反射联动；**ModelComponent** 持有 ResourceId（modelResourceId），Entity创建时自动从Scene节点同步 |
