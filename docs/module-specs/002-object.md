...
- **反射**：类型注册、类型信息查询、属性/方法枚举、RTTI 扩展。
- **序列化**：二进制/文本序列化、版本化、引用解析、默认值。
...
- **内存**：类型描述符、属性表、序列化缓冲。
- **无文件/GPU 直接操作**：通过抽象层与 Resource 等模块配合完成资源引用与元数据。**资源描述类型（AssetDesc）**由 002 定义或注册；013 反序列化时调用 002 得到 LevelAssetDesc、MaterialAssetDesc 等，再交 004/011 等“仅用内存”初始化。
