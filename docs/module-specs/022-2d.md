...
对外接口以 **ABI** 为准：`specs/_contracts/022-2d-ABI.md`；契约：`specs/_contracts/022-2d-public-api.md`。

- **精灵**：ISprite::Load、GetAtlas、GetSlice、SetSortOrder、GetSortOrder、SubmitToPipeline；CreateSprite；与 Pipeline 2D 绘制对接。
- **Tilemap**：ITilemap::GetTileSet、GetLayerCount、GetLayer、Draw、SetCollisionLayer、GetCollisionLayer；CreateTilemap；与 Pipeline Tilemap 绘制。
- **2D 物理**：Create2DBody、Create2DShape；与 014-Physics 2D 碰撞/刚体对接。
- **2D 渲染**：ICamera2D、SortingLayer；与 RenderCore/Pipeline 的 2D Pass。
...
Sprite：ISprite::Load、GetAtlas、GetSlice、SetSortOrder、GetSortOrder、SubmitToPipeline；CreateSprite。  
Tilemap：ITilemap::GetTileSet、GetLayerCount、GetLayer、Draw、SetCollisionLayer、GetCollisionLayer；CreateTilemap。  
Physics2D：Create2DBody、Create2DShape、与 Physics 对接。  
Rendering2D：ICamera2D、SortingLayer、与 RenderCore 对接。
