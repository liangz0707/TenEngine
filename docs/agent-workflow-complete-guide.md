...
本文档提供**规约预置要求**与**两套工作流**：**通用模块流程**（模块首次落地 / 首个 feature）、**功能迭代流程**（同模块后续 feature 迭代）。执行前请确认主仓库为 `TenEngine/`（项目根目录），worktree 为 `TenEngine-NNN-modulename/`。
...
**执行位置**：步骤 0.0、0.1 在主仓库（项目根目录 `TenEngine/`）；0.2 及步骤 1～8 在**当前 worktree**（`TenEngine-NNN-modulename/`）。步骤 4 写回契约时需切回主仓库 T0-contracts。
...
```
当前在主仓库（项目根目录），请切换到 T0-contracts 分支并拉取最新。
...
```
请按 docs/agent-workflow-complete-guide.md「完整指令流程」与「二、工作流」顺序执行：engine.contracts-pull → specify → clarify(可选) → plan → engine.contracts-writeback → tasks → implement → 合并到 T0-NNN-modulename 推送。主仓库为项目根目录（TenEngine/），worktree 为 TenEngine-NNN-modulename/。规约与契约见该文档「一、三」。<本切片范围> 替换为示例切片或规约自填。提交信息使用英文。
```
