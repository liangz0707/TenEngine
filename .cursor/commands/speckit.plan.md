...
3. **Execute plan workflow**: Follow the structure in IMPL_PLAN template to:
   ...
   - **实现范围 (TenEngine)**：本 feature **只实现 ABI 文件**（`specs/_contracts/NNN-modulename-ABI.md`）中列出的符号与能力；不得设计或实现 ABI 未声明的对外接口。对外接口以 ABI 文件为准；契约（public-api）描述能力与类型，接口符号与签名以 ABI 为准。见 `specs/_contracts/README.md`、`.specify/memory/constitution.md` §VI。
...
- **实现范围**：仅实现 **ABI 文件**（`specs/_contracts/NNN-modulename-ABI.md`）中列出的符号与能力；不实现 ABI 未声明的对外接口。plan 产出的「契约更新」须同步到 ABI 文件（完整 ABI 条目），见 `docs/agent-workflow-complete-guide.md`「2.0 写回契约」。
