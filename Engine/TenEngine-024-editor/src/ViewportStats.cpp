...
public:
  ...
  {
    // Initialize history vectors with 60 elements
    m_cpuTimeHistory.resize(60, 0.0f);
    m_gpuTimeHistory.resize(60, 0.0f);
  }
  ...
  void EndFrame() override {
    ...
    // Update history
    int idx = m_historyIndex % 60;
    m_cpuTimeHistory[idx] = m_stats.timing.cpuTimeMs;
    m_gpuTimeHistory[idx] = m_stats.timing.gpuTimeMs;
    m_historyIndex++;

    m_stats.totalTime += frameTime / 1000.0f;
  }
  ...
  void AddDrawCall(int triangles, int vertices) override {
  ...
  void ResetStats() override {
    m_stats = ViewportStatistics{};
    m_historyIndex = 0;
    for (auto& v : m_cpuTimeHistory) v = 0.0f;
    for (auto& v : m_gpuTimeHistory) v = 0.0f;
  }
  ...
  void SetPosition(float x, float y) override {
    m_settings.position[0] = x;
    m_settings.position[1] = y;
  }
  ...
  void OnDraw() override {
    ...
    ImVec2 windowPos(m_settings.position[0], m_settings.position[1]);
...
private:
  void DrawStatsContent() {
    ...
    if (m_settings.showDrawCalls) {
      ImGui::Text("Draw Calls: %d", m_stats.render.drawCalls);
    }

    if (m_settings.showTriangleCount) {
      ImGui::Text("Triangles: %d", m_stats.render.triangles);
    }

    if (m_settings.showVertexCount) {
      ImGui::Text("Vertices: %d", m_stats.render.vertices);
    }

    if (m_settings.showMemoryUsage) {
      float vramMB = static_cast<float>(m_stats.render.vramUsed) / (1024.0f * 1024.0f);
      float budgetMB = static_cast<float>(m_stats.render.vramBudget) / (1024.0f * 1024.0f);
    ...
    if (m_settings.showEntityCount) {
      ImGui::Text("Entities: %d", m_stats.scene.entityCount);
    }

    if (m_settings.showLightCount) {
      ImGui::Text("Lights: %d", m_stats.scene.lightCount);
    }
    ...
    if (m_settings.showPhysicsStats) {
      ...
      ImGui::Text("  Bodies: %d active, %d sleeping",
                  m_stats.physics.activeBodies, m_stats.physics.sleepingBodies);
      ImGui::Text("  Contacts: %d", m_stats.physics.activeContacts);
    }
