...
public:
  ...
  {
    m_boundsMin = te::math::Vec3(0.0f);
    m_boundsMax = te::math::Vec3(0.0f);
  ...
  void SetComponentCount(const char* typeName, int count, int enabledCount) override {
    ...
    for (auto& stat : m_componentStats) {
      if (stat.typeName && std::string(stat.typeName) == typeName) {
        stat.count = count;
        if (enabledCount >= 0) {
  ...
  void SetSceneBounds(te::math::Vec3 const& min, te::math::Vec3 const& max) override {
    m_boundsMin = min;
    m_boundsMax = max;
  }
  
  void GetSceneBounds(te::math::Vec3& min, te::math::Vec3& max) const override {
    min = m_boundsMin;
    max = m_boundsMax;
  }
  
  te::math::Vec3 GetSceneCenter() const override {
    return (m_boundsMin + m_boundsMax) * 0.5f;
  }
  
  te::math::Vec3 GetSceneExtents() const override {
    return (m_boundsMax - m_boundsMin) * 0.5f;
  }
...
private:
  ...
  void DrawComponentStats() {
    ...
    if (ImGui::BeginTable("ComponentTable", 3, 
      ...
      for (auto const& stat : sorted) {
        ...
        ImGui::Text("%s", stat.typeName ? stat.typeName : "Unknown");
  ...
  void DrawSceneBounds() {
    ...
    te::math::Vec3 center = GetSceneCenter();
    te::math::Vec3 extents = GetSceneExtents();
    te::math::Vec3 size = m_boundsMax - m_boundsMin;
  ...
  // Scene bounds
  te::math::Vec3 m_boundsMin;
  te::math::Vec3 m_boundsMax;
