...
public:
  ...
  uint64_t CreatePrefabFromEntity(te::entity::EntityId entityId, 
    ...
    PrefabData data;
    data.id = prefabId;
    std::strncpy(data.path, path, sizeof(data.path) - 1);
    ...
    if (lastSlash != std::string::npos && lastDot != std::string::npos) {
      std::string name = pathStr.substr(lastSlash + 1, lastDot - lastSlash - 1);
  ...
  te::entity::EntityId InstantiatePrefab(uint64_t prefabId,
    ...
    if (it == m_prefabs.end()) {
      ...
      return te::entity::InvalidEntityId;
    }
    ...
    te::entity::EntityId instanceId = static_cast<te::entity::EntityId>(m_nextPrefabId + 10000);
    m_instances[instanceId] = instance;
    
    te::core::Log(te::core::LogLevel::Info, 
                  ("PrefabSystem: Instantiated prefab " + std::string(it->second.name)).c_str());
  ...
  PrefabData const* GetPrefabByName(char const* name) const override {
    ...
    for (auto const& pair : m_prefabs) {
      if (std::strcmp(pair.second.name, name) == 0) {
        return &pair.second;
      }
    }
    return nullptr;
  ...
  bool SavePrefab(uint64_t prefabId) override {
    ...
    // For now, just log
    te::core::Log(te::core::LogLevel::Info, 
                  ("PrefabSystem: Saved prefab to " + std::string(it->second.path)).c_str());
    return true;
  }
  ...
  uint64_t LoadPrefab(char const* path) override {
    ...
    for (auto const& pair : m_prefabs) {
      if (std::strcmp(pair.second.path, path) == 0) {
        return pair.first;
      }
    }
...
private:
  ...
  std::map<te::entity::EntityId, PrefabInstance> m_instances;
  std::map<te::entity::EntityId, std::vector<PrefabOverride>> m_overrides;
