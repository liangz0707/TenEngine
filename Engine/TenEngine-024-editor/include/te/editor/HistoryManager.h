...
/**
 * @brief History action type.
 */
enum class HistoryActionType {
  Transform,         ///< Transform change
  PropertyChange,    ///< Property modification
  Create,            ///< Object creation
  Delete,            ///< Object deletion
  ParentChange,      ///< Hierarchy change
  ComponentAdd,      ///< Component added
  ComponentRemove,   ///< Component removed
  Multiple,          ///< Compound action
  Custom             ///< Custom action
};

/**
 * @brief History action record.
 */
struct HistoryAction {
  ...
  // Target info
  uint64_t targetId = 0;       ///< Entity/Component/Resource ID
  const char* propertyName = nullptr;
  
  // State data (simplified - would use variant in production)
  bool hasOldData = false;
  bool hasNewData = false;
  
  // Callbacks
  std::function<bool()> undo;   ///< Undo function
  std::function<bool()> redo;   ///< Redo function
  
  // Compound actions
  std::vector<uint64_t> subActions;
};

/**
 * @brief History bookmark for jumping to specific states.
 */
