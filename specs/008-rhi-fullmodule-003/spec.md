...
**Feature Branch**: `008-rhi-fullmodule-003`  
**Created**: 2026-01-31  
**Status**: Draft  
...
- **本模块范围**（本 feature 实现完整模块内容）：
  ...
  4. **PSO**：CreateGraphicsPSO、CreateComputePSO、SetShader、Cache、DestroyPSO；接受**后端原生 Shader 字节码**（SPIR-V/DXIL/MSL），由 010-Shader 按所选后端产出。
  5. **Synchronization**：CreateFence、CreateSemaphore、Wait、Signal、DestroyFence、DestroySemaphore、ResourceBarrier。
  6. **SwapChain/Present**：本 feature 包含 SwapChain、Present（及可选 XR 对接点）；与 020-Pipeline、027-XR 的边界以契约为准。
  7. **后端与调试**：首期实现 **Vulkan、D3D12、Metal** 全部三个后端；集成 **Vulkan Validation Layer、D3D12 Debug Layer** 等，通过构建/运行时选项启用。

实现时只使用**本 feature 依赖的上游契约**（`specs/_contracts/001-core-public-api.md`）中已声明的类型与 API；不实现本规约未列出的能力。

- **ABI 与构建规约**（`.specify/memory/constitution.md` §VI）：本模块须实现其 **ABI 文件**（`specs/_contracts/008-rhi-ABI.md`）中的**全部**符号与能力；构建须通过**引入真实子模块源码**（如 CMake `add_subdirectory`）满足依赖，**禁止**使用 stub、mock 或与契约不一致的代替实现作为长期方案。**契约更新**：接口变更须在对应 **ABI 文件**中**增补或替换**对应的 ABI 条目（plan 只产出新增/修改部分，写回也仅写入该部分）；下游所需接口须在**上游模块的 ABI 文件**中以 **TODO** 登记。详见 `specs/_contracts/README.md`「契约更新流程」。

- **第三方依赖**：第三方库引入说明在契约 `specs/_contracts/008-rhi-public-api.md` 中声明（「第三方依赖」小节）；本 spec 引用该契约即可，不在 spec 中重复列出。Plan 从 public-api 读取并自动填入「第三方依赖」小节，Task 将生成版本选择、自动下载、配置、安装、编译测试、部署、配置实现等任务。详见 `docs/third_party-integration-workflow.md`。

## Clarifications

### Session 2026-01-31

- Q: 本 feature 首期是否包含 SwapChain/Present/XR？ → A: 包含；本 feature 实现 SwapChain/Present（及可选 XR 对接点）。
- Q: 首期实现的后端范围？ → A: 全部；Vulkan、D3D12、Metal 均在首期实现。
- Q: ResourceBarrier 的接口粒度？ → A: 细粒度；按资源 + 状态过渡（如 BufferBarrier/TextureBarrier、srcState/dstState）。
- Q: PSO 接受的 Shader 字节码格式？ → A: 后端原生（SPIR-V/DXIL/MSL），由 010-Shader 按后端产出。
- Q: Validation/Debug Layer 是否在本 feature 中集成？ → A: 本 feature 集成；Vulkan Validation、D3D12 Debug Layer 等，构建/运行时选项启用。
...
- ResourceBarrier 使用细粒度接口；上层须正确声明资源状态 OldState→NewState，错误声明可能导致未定义行为。
...
- **FR-002**: 系统必须提供命令列表的录制与提交：Begin/End、Draw、Dispatch、Copy、**ResourceBarrier（细粒度：按资源+状态过渡，如 BufferBarrier/TextureBarrier、srcState/dstState）**、Submit，语义明确。
- **FR-003**: 系统必须提供 Buffer、Texture、Sampler、视图的创建与销毁；内存管理与生命周期明确，失败时有明确报告。
- **FR-004**: 系统必须提供图形/计算 PSO 的创建、与 **后端原生 Shader 字节码**（SPIR-V/DXIL/MSL，由 010-Shader 按后端产出）绑定、可选缓存与编译；与 RenderCore/Shader 对接。
...
## Threading (FR-007) *(T085)*

RHI 接口在多线程访问下的行为由**实现定义并文档化**；本 feature 不要求默认并发安全。

- **通用**：IDevice、IQueue、ICommandList 等句柄的并发访问由调用方保证；Create*/Destroy* 失败返回 nullptr，不自动回退。
- **Vulkan**：VkDevice/VkQueue 的线程约束见 Vulkan spec（如 vkQueueSubmit 可从多线程调用；同一 VkCommandBuffer 不得并发录制）；本实现中命令缓冲按「单次录制周期」使用，未做额外加锁。
- **D3D12**：ID3D12CommandList 录制须单线程；ID3D12CommandQueue::ExecuteCommandLists 的并发见 D3D12 文档；本实现未做额外同步。
- **Metal**：MTLCommandBuffer 的编码与 commit 约束见 Apple 文档；本实现中 MTLCommandBuffer 在 Submit 时 commit，未做跨线程共享保证。
