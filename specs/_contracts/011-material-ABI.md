...
| 011-Material | te::material | MaterialResource | 类 | 材质资源实现 | te/material/MaterialResource.h | 实现 resource::IMaterialResource；Load（JSON 解析、按 shader GUID 取 IShaderResource、按反射 name→(set,binding) 解析贴图 GUID）、Save（写 JSON）、Import 空实现；GetTextureRefs 输出 (MaterialTextureSlot, GUID 字符串)；析构对 IShaderResource*/ITextureResource* 调用 Release；**SetDevice(IDevice*)** 设置 RHI 设备供 EnsureDeviceResources 使用；**EnsureDeviceResources** 对贴图链 Ensure 后，根据 shader 反射创建 IUniformLayout/IUniformBuffer 并 Update(paramBuffer_)；**GetUniformBuffer()** 返回 te::rendercore::IUniformBuffer*（可为 nullptr）；**IsDeviceReady** 重写为检查 device、deviceResourcesReady_ 及所有贴图 IsDeviceReady |
