...
- **命名与签名约定**：**成员方法采用首字母大写的驼峰命名（PascalCase）**；**所有方法在说明列须给出完整函数签名**（含返回类型、参数列表）。
...
| 导出形式 | 类名列 | 符号列示例 | 调用方式 |
|----------|--------|------------|----------|
| **自由函数** | — | Alloc, Free, Log | 命名空间::函数名(…)；**PascalCase** |
...
| **静态方法** | 类名 | TypeRegistry::RegisterType | 命名空间::类名::方法(…)；**PascalCase** |
| **单例** | 类名 | Engine::Instance(), TypeRegistry::Instance() | 先取实例再调成员，或 类名::Instance()->Method()；**PascalCase** |
| **模板类/函数** | 类名或 — | Array&lt;T&gt;, clamp&lt;T&gt; | 符号写 类型名&lt;T&gt; 或 函数名&lt;T&gt;；说明中写约束与特化约定 |
| **模板方法** | 类名 | Registry::GetSubsystem&lt;T&gt;() | 类名::方法&lt;T&gt;(…)；**PascalCase**；说明中写约束 |
...
- **接口须在 ABI 文件中以完整条目更新**：模块对外接口的变更**必须**在本模块的 ABI 文件（本索引所引用的 `NNN-modulename-ABI.md`）中体现为**完整的 ABI 表行**（含符号与说明/完整函数签名）；不得仅更新 `NNN-modulename-public-api.md` 而 ABI 表不更新或只写占位。
- **下游所需接口在上游 ABI 中以 TODO 登记**：若**下游模块**需要某接口而**上游模块**尚未实现，须在**上游模块的 ABI 文件**中增加该接口的 **TODO** 行（在说明列标明「TODO：下游 NNN-xxx 需要」及拟议签名），待上游实现时将该行转为正式 ABI 条目并移除 TODO。下游不得依赖未在上游 ABI 中登记（含 TODO）的接口。

详见 **`specs/_contracts/README.md`**「契约更新流程」与 **`docs/agent-interface-sync.md`** §4.2。

## ABI 实现与构建要求（强制）

- **完整 ABI 实现**：各模块**必须**实现其 ABI 文件（本索引所引用的 `NNN-modulename-ABI.md`）中列出的**全部**符号与能力；不得仅实现部分，或长期保留「待补充」条目而不落地实现。
- **构建须引入子模块代码**：构建过程**必须**通过引入**真实子模块源码**（如 CMake `add_subdirectory`、`FetchContent` 拉取对应模块仓库）来满足对上游模块的依赖；**不得**在未引入上游模块源码的情况下，用本模块内自写的 stub、mock 或占位实现代替上游模块。
- **禁止 stub/代替方案**：**禁止**为通过编译或联调而提供仅返回空/默认值的 stub 实现、或与 ABI/契约不一致的代替实现作为长期方案；仅允许在**明确标注为临时、且有计划替换为真实实现**的过渡期使用占位，且须在任务/计划中跟踪替换。

以上与 **`specs/_contracts/README.md`**「ABI 实现与构建要求」、**`.specify/memory/constitution.md`** §VI 及 **`docs/engine-build-module-convention.md`** 一致。
