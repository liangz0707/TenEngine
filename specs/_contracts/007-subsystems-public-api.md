...
- **实现方**：007-Subsystems（L1；可插拔子系统：描述符、注册、Start/Stop；Display、XR 等）
...
| ISubsystem / GetSubsystem\<T\> | 子系统实例、按类型/接口查询 | 与 Start/Stop 一致 |
...
- 须在 Core、Object 可用之后使用；子系统启动/停止顺序按依赖图保证。XR、Display 等作为实现挂接于本模块。
