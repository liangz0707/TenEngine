...
| 名称 | 语义 | 生命周期 |
|------|------|----------|
| TextureHandle | 贴图句柄；CreateTexture(desc) 仅接受内存；EnsureDeviceResources 时 028 经 030 调用 008 创建 DResource | 创建后直至显式释放 |
...
| 2 | EnsureDeviceResources | 按需经 030 调用 008-RHI 创建 RHI 纹理（DResource） |
| 3 | 格式/尺寸查询 | GetFormat、GetWidth、GetHeight、GetMipCount、GetPixelData、GetPixelDataSize |
| 4 | ReleaseTexture | 释放 TextureHandle 及关联内存；GPU 纹理需先 DestroyDeviceTexture |
| 5 | 图像导入 | ImageImporterRegistry 单例；DetectFormat、ImportImage；可选 StbImageImporter（兜底）、LibPngImporter、LibJpegTurboImporter、LibWebPImporter（由 CMake 选项启用） |
...
- 须在 Core、Object、RHI、RenderCore、Resource、030-DeviceResourceManager 初始化之后使用。013 解析得到像素/描述后调用 028 CreateTexture；DResource 在下游触发 EnsureDeviceResources 时由 028 经 030 调用 008 创建。
...
- [x] **TextureHandle**：内持 RHI 纹理（DResource）、格式/尺寸信息。
- [x] **接口**：CreateTexture(desc)→TextureHandle*；EnsureDeviceResources(handle, device)；ReleaseTexture(handle)；GetFormat/GetWidth/GetHeight/GetMipCount；经 030 调 008。
