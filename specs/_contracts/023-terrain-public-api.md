...
下游仅通过上述类型与句柄访问；与 Resource 地形块/流式、Mesh 顶点格式、Pipeline 地形 Pass、RenderCore 格式对接。**ABI 显式表**：[023-terrain-ABI.md](./023-terrain-ABI.md)。

## 能力列表（提供方保证）

1. **TerrainData**：ITerrain::GetHeightMap、GetLayerCount、GetLayer、GetDetailMap、RequestStreaming；CreateTerrain；与 Resource 流式对接。
2. **LOD**：ITerrainPatch::GetLODLevel、SetLODLevel；ITerrain::GetPatch、SelectLOD、StreamBlock；LODLevel；与 Resource 对接。
3. **MeshGen**：GeneratePatch、VertexFormat；与 Mesh/RenderCore 对接。
4. **Painting（可选）**：IPaintBrush::PaintHeight、PaintTexture；PaintBrushParams；与 Editor 对接。
...
| 2026-01-28 | 根据 023-terrain-ABI 反向更新：ITerrain、ITerrainPatch、CreateTerrain、GeneratePatch、IPaintBrush、PaintBrushParams；能力与类型与 ABI 表一致 |
