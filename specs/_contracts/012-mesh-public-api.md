...
- **实现方**：012-Mesh（L2；网格数据、LOD、蒙皮、顶点/索引；MeshResource 持 CPU 数据；统一更新时机由 PrepareRenderMesh → GetOrCreateRenderer→Update 完成 CPU→GPU 上传）
...
| 5 | AABB | GetMeshAABB、GetSubmeshAABB；局部空间包围盒，用于视锥剔除 |
| 6 | Resource | MeshResource 实现 IResource/IMeshResource；仅持 CPU 数据（MeshHandle）；支持 Load、LoadAsync、Save、Import；GetMeshHandle、GetVertexData、GetIndexData、GetVertexDataSize、GetIndexDataSize；IsDeviceReady 表示 Load 成功 |
| 7 | BuiltinMeshes | 与 MeshResource 同级；GetFullscreenQuadMesh、GetSphereMesh、GetHemisphereMesh、GetPlaneMesh、GetQuadMesh、GetTriangleMesh、GetCubeMesh、GetConeMesh；返回 MeshResource const*（内部缓存，调用方不 delete） |
| 8 | Import | 支持从外部格式导入：OBJ（fast_obj）、glTF/glB（cgltf）、通用格式（Assimp）；通过 MeshResource::Import 调用；需要编译定义 TENENGINE_USE_FAST_OBJ、TENENGINE_USE_ASSIMP、TENENGINE_USE_CGLTF |
| 9 | Serialization | MeshAssetDesc 序列化/反序列化（002-Object）；.mesh 文件存储元数据，.meshdata 文件存储二进制数据；MeshDeserializer::Deserialize、SerializeMeshToBuffer |
| 10 | ModuleInit | InitializeMeshModule(manager) 注册资源工厂到 ResourceManager |
...
| 2026-02-11 | Skin 运行时缓冲：SkinMatrixBufferHandle、CreateSkinMatrixBuffer、ReleaseSkinMatrixBuffer、GetSkinMatrixBuffer、GetSkinMatrixBufferSize |
| 2026-02-22 | 同步代码：移除 MeshRenderer 相关内容（待实现）；更新能力 5 为 AABB；补充 GetVertexStride、GetIndexFormat；更新能力 10 为 ModuleInit；更新 MeshResource 方法列表 |
