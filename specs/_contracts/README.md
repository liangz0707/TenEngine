...
- **依赖关系总览**：[000-module-dependency-map.md](./000-module-dependency-map.md)（29 模块依赖表与上下游）。
...
| [004-scene-public-api.md](./004-scene-public-api.md) | 004-Scene 对外 API（场景图、节点、空间、CreateSceneFromDesc；不发起加载） | 004-Scene | 005-Entity, 014-Physics, 020-Pipeline, 024-Editor, 029-World 等 |
| [005-entity-public-api.md](./005-entity-public-api.md) | 005-Entity 对外 API（实体、组件、ModelComponent 与 Model 引用、层级、查询；Entity-Node 1:1） | 005-Entity | 014-Physics, 015-Animation, 020-Pipeline, 024-Editor, 026-Networking 等 |
...
| [008-rhi-public-api.md](./008-rhi-public-api.md) | 008-RHI 图形抽象层对外 API（RCI 即 RHI；设备、命令列表、资源、PSO、同步） | 008-RHI | 009-RenderCore, 010-Shader, 012-Mesh（EnsureDeviceResources）, 019-PipelineCore, 020-Pipeline, 024-Editor, 027-XR |
...
| [011-material-public-api.md](./011-material-public-api.md) | 011-Material 对外 API（材质定义、CreateMaterial、参数、实例、与 Shader/PSO 绑定；不发起加载） | 011-Material | 013-Resource（CreateMaterial）, 020-Pipeline |
| [012-mesh-public-api.md](./012-mesh-public-api.md) | 012-Mesh 对外 API（CreateMesh、顶点/索引、子网格、LOD、蒙皮；DResource 在 EnsureDeviceResources 时创建） | 012-Mesh | 013-Resource（CreateMesh）, 020-Pipeline, 023-Terrain, 015-Animation |
| [028-texture-public-api.md](./028-texture-public-api.md) | 028-Texture 对外 API（CreateTexture、格式、Mipmap、EnsureDeviceResources；贴图句柄；013 加载后交 028 创建） | 028-Texture | 011-Material（贴图引用）, 013-Resource（CreateTexture）, 020-Pipeline, 021-Effects, 022-2D, 023-Terrain, 024-Editor |
| [013-resource-public-api.md](./013-resource-public-api.md) | 013-Resource 对外 API（导入、Load、EnsureDeviceResources、卸载、流式、可寻址；ResourceId、LoadHandle、Metadata；013 为唯一 Load 入口，不创建 DResource） | 013-Resource | 010-Shader, 011-Material（CreateMaterial）, 012-Mesh（CreateMesh）, 016-Audio, 020-Pipeline, 022-2D, 023-Terrain, 024-Editor, 028-Texture（CreateTexture）, 029-World（Level 加载） |
...
| [020-pipeline-public-api.md](./020-pipeline-public-api.md) | 020-Pipeline 对外 API（管线上下文、渲染目标、DrawCall、可见集/批次；与 Scene/Entity/Resource 的渲染资源取用关系） | 020-Pipeline | 021-Effects, 022-2D, 023-Terrain, 024-Editor, 027-XR |
...
| [029-world-public-api.md](./029-world-public-api.md) | 029-World 对外 API（场景资源管理、Level 句柄、CreateLevelFromDesc 调 004 CreateSceneFromDesc；底层依赖 Scene） | 029-World | 020-Pipeline, 024-Editor（可选） |

**说明**：**每模块一契约**，共 29 份模块契约 + 1 份边界契约（pipeline-to-rci）；对应规格见 `docs/module-specs/`。新增或修改契约时请在本 README 和 `000-module-dependency-map.md` 中同步更新。
