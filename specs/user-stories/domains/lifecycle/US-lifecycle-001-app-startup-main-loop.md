# US-lifecycle-001：应用启动并进入主循环

- **标题**：应用启动并进入主循环
- **编号**：US-lifecycle-001（原 US-001）
...
5. **主循环**：每帧执行「轮询事件 → 更新 → 渲染」（渲染细节见 rendering 领域）；循环由 Application 或上层驱动。
...
- Unity：Subsystems（SubsystemDescriptor、IntegratedSubsystem Start/Stop）、[SubsystemsModule](https://docs.unity3d.com/ScriptReference/UnityEngine.SubsystemsModule.html)。
- Unreal：Module LoadingPhase / Type、FModuleManager。

*ABI 条目已同步到 `specs/_contracts/001-core-ABI.md`、`003-application-ABI.md`、`007-subsystems-ABI.md`。*
