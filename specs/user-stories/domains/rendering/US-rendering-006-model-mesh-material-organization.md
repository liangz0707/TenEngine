...
| 模块 | 职责摘要 |
|------|----------|
| 013-Resource | Model/Mesh/Material 统一加载；返回 IModelResource（类型归属 029-World，引用 Mesh、Material）、IMeshResource（OBJ/FBX 等）、IMaterialResource（引擎格式，保存 Shader，引用贴图、参数） |
...
以下与 **specs/_contracts/013-resource-ABI.md**、**029-world-public-api.md**、011-Material、012-Mesh 契约一致，无需重复新增表项；确保：

- **IModelResource**（归属 029-World）：硬盘上的 Model 资源**引用了 Material 和 Mesh**；提供对若干 Mesh、若干 Material 的引用访问。
