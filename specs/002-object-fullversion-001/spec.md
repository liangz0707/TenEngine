# Feature Specification: 002-Object Full Feature Set

**Feature Branch**: `002-object-fullversion-001`  
**Created**: 2026-01-29  
**Status**: Draft  
**Input**: 本 feature 的完整模块规约见 `docs/module-specs/002-object.md`，对外 API 契约见 `specs/_contracts/002-object-public-api.md`。本 feature 需实现完整功能集；spec.md 中引用规约与契约，只描述本切片范围（即完整功能集）。
...
- **本切片范围（完整功能集，显式枚举）**：
  ...
  2. **序列化**：序列化器抽象、二进制与文本格式均支持、版本迁移（必选）、对象引用与 GUID 解析；往返等价性可验证。
  3. **属性系统**：属性描述、元数据、默认值、范围/枚举约束；与反射和序列化联动。
  4. **类型注册**：注册表、按模块注册、类型工厂 CreateInstance、生命周期；与 Core 模块加载协调。

实现时只使用 `specs/_contracts/001-core-public-api.md` 已声明的类型与 API；对外暴露与契约能力列表及类型与句柄一致的全部 API。

## Clarifications

### Session 2026-01-29

（以 `docs/module-specs/002-object.md` 与 `specs/_contracts/002-object-public-api.md` 为准，仅针对本切片范围澄清。）

- Q: 本 feature 中「版本迁移」为必选还是可选？ → A: 必选。契约能力列表明确包含「版本迁移」；完整功能集须与契约一致，故版本迁移在本切片内为必选能力。
- Q: 重复注册相同 TypeId/类型名时行为如何约定？ → A: 重复 TypeId 拒绝（保证唯一性）；重复类型名由实现约定（覆盖或拒绝），须在实现/契约中明确并可测。
- Q: 序列化格式须支持二进制、文本或两者？ → A: 两者均支持。规约与契约均写「二进制/文本序列化」与「二进制/文本格式」，完整功能集须同时支持两种格式（或通过抽象可扩展至两者）。
- Q: GUID 与 013-Resource 的引用格式由谁约定？ → A: 按契约「须与 013-Resource 等消费者约定引用格式」，引用格式在集成或跨模块契约中约定；本 feature 保证 GUID 可解析、可扩展，具体格式与 013-Resource 对齐留待集成阶段。
...
**Independent Test**: 序列化含嵌套引用与 GUID 的对象图，反序列化后验证引用与数据等价；可验证版本迁移行为（若在本切片内）。
...
3. **Given** 旧版本序列化数据，**When** 使用版本迁移（若本切片实现），**Then** 可升级到当前格式或行为有定义。
...
- 重复注册相同 TypeId/类型名：重复 TypeId 拒绝（保证唯一性）；重复类型名由实现约定（覆盖或拒绝），须在契约或实现中明确并可测。
...
- **FR-002**: 系统必须提供完整序列化能力：序列化器抽象、二进制与文本格式均支持、版本迁移（必选）、对象引用与 GUID 解析；往返等价性可验证。
...
- **FR-005**: 系统必须仅使用 `specs/_contracts/001-core-public-api.md` 已声明的类型与 API；对外暴露与 `specs/_contracts/002-object-public-api.md` 能力列表及类型与句柄一致的全部 API。
...
- **SC-005**: 实现与契约能力列表、类型与句柄一致；仅依赖上游契约已声明的 API。
...
- **001-Core**：`specs/_contracts/001-core-public-api.md`。实现时只使用契约中已声明的类型与接口（基础类型、容器、字符串、内存分配、日志等）。
- **GUID 与下游约定**：GUID 引用格式与 013-Resource 等消费者的约定留待集成或跨模块契约；本 feature 保证 GUID 可解析、可扩展，具体格式与下游对齐在集成阶段完成。
